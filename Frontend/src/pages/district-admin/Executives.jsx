...
export function DistrictExecutives() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">District Field Executives</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Field executives and operational supervisors across {user?.district || 'Salem'} District.
        </p>
      </div>

      <DataTable
        title="Operations Executive Staff"
        subtitle="Manage field officers assigned within district boundaries"
        columns={columns}
        data={execs}
        loading={loading}
        onRefresh={loadData}
