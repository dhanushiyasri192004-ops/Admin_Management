...
export function DistrictKYC() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">District KYC Compliance Queue</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Identity document verifications originating within {user?.district || 'Salem'} District.
        </p>
      </div>

      <DataTable
        title="KYC Verification Queue"
        subtitle="Manage compliance and identity auditing in district"
        columns={columns}
        data={kycList}
        loading={loading}
        onRefresh={loadData}
        searchPlaceholder="Search applicant name or doc..."
        exportFileName="district_kyc.csv"
      />
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              onClick={() => handleUpdate(selectedRecord?.id, 'Rejected', 'Document rejected by District Admin')}
            ...
            <button
              onClick={() => handleUpdate(selectedRecord?.id, 'Approved', 'Clearance approved by District Admin')}
