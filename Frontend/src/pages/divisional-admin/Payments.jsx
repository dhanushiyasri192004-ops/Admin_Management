...
export function DivisionalPayments() {
  ...
  const agentColumns = [
    ...
    },
    {
      header: 'Pincode Zone',
      accessor: 'pincode',
      render: (row) => <span className="font-mono text-emerald-400 text-xs">PIN: {row.pincode}</span>
    },
    {
      header: 'Status',
      accessor: 'status',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-white">Division Payment Authorizations</h2>
          <p className="text-xs text-slate-400">Review claims and payouts for agents & vendors in this division.</p>
        </div>
      ...
      {tab === 'agents' ? (
        <DataTable
          title="Division Agent Claims"
          subtitle="Scoped to assigned division"
          columns={agentColumns}
          data={agentPayments}
          loading={loading}
          onRefresh={loadData}
          exportFileName="divisional_agent_payments.csv"
        />
      ) : (
        <DataTable
          title="Division Vendor Payouts"
          subtitle="Scoped to assigned division"
          columns={vendorColumns}
          data={vendorPayments}
          loading={loading}
          onRefresh={loadData}
          exportFileName="divisional_vendor_payments.csv"
        />
      )}
