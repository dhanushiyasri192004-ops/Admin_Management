...
export function DivisionalTechnicians() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Division Technicians Roster</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Field service specialists assigned within {user?.division || 'Salem North'} Division pincodes.
        </p>
      </div>

      <DataTable
        title="Field Service Engineers"
        subtitle="Manage technician assignments within assigned division"
        columns={columns}
        data={techs}
        loading={loading}
        onRefresh={loadData}
        searchPlaceholder="Search technician..."
        exportFileName="division_technicians.csv"
      />
    </div>
  );
}
