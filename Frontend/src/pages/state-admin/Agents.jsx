...
export function StateAgents() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">State Agents Network</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Customer onboarding agents and field sales partners across the State.</p>
      </div>

      <DataTable
        title="Agent Network Roster"
        subtitle="Referral volumes and commission balances across all districts"
        columns={columns}
        data={agents}
        loading={loading}
        onRefresh={loadData}
        searchPlaceholder="Search agent by name or phone..."
        exportFileName="state_agents.csv"
      />
    </div>
  );
}
