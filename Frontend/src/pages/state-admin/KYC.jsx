...
import { FileCheck2, ExternalLink, Check, X } from 'lucide-react';
...
export function StateKYC() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">State KYC Compliance Desk</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Identity verification requests and document clearance across the State.</p>
      </div>

      <DataTable
        title="KYC Verification Queue"
        subtitle="Review identity documents for vendors, customers, and field agents"
        columns={columns}
        data={kycList}
        loading={loading}
        onRefresh={loadData}
        searchPlaceholder="Search applicant name or doc number..."
        exportFileName="state_kyc.csv"
      />
      ...
      >
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              onClick={() => handleUpdate(selectedRecord?.id, 'Rejected', 'Document rejected during audit')}
            ...
            <button
              onClick={() => handleUpdate(selectedRecord?.id, 'Approved', 'Clearance approved by State Admin')}
