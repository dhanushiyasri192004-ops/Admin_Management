...
import { ShoppingBag, MapPin, IndianRupee } from 'lucide-react';
...
export function StateOrders() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          <div className="text-xs text-slate-300">{row.district} / {row.division}</div>
      ...
      render: (row) => (
        <div>
          ...
          {row.discountAmount > 0 && (
            <div className="text-[10px] text-slate-400">Card Discount: -₹{row.discountAmount}</div>
          )}
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-white">State Orders Overview</h2>
        <p className="text-xs text-slate-400">All customer orders dispatched and delivered across the state.</p>
      </div>
