...
export function StateSupportTeam() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">State Customer Support Queue</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Escalations and customer service inquiries across all State districts.</p>
      </div>

      <DataTable
        title="Support Desk Queue"
        subtitle="Manage customer assistance inquiries and resolution timelines"
        columns={columns}
        data={tickets}
        loading={loading}
        onRefresh={loadData}
        searchPlaceholder="Search ticket ID or subject..."
        exportFileName="state_support_tickets.csv"
      />
    </div>
  );
}
